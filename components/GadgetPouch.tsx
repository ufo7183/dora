...
export const GadgetPouch: React.FC = () => {
    ...
    const handleGenerate = async () => {
        ...
        try {
             // Step 1: Analyze text to get prompts
            const analysisResponse = await genAI.models.generateContent({
                model: 'gemini-2.5-flash',
                contents: `Analyze the following text and generate 5 distinct, detailed, and visually rich prompts for an image generation model. Each prompt should capture a key scene, moment, or theme from the text. The prompts must be in English. For any characters mentioned, specify their ethnicity as Taiwanese, Japanese, or Korean.
...
Text: "${text}"`,
    ...
    const handleDownloadSelected = () => {
        generatedImages.forEach((image, index) => {
            if (image.selected) {
                downloadImage(image.src, `pouch-image-${index + 1}.jpg`);
            }
        });
    };
    ...
    return (
        <div className="w-full h-full flex flex-col p-4 md:p-8 pt-16 bg-gray-100 gap-4 overflow-y-auto">
            ...
            <header className="text-center">
                 <h2 className="text-3xl font-bold text-blue-800">百寶袋</h2>
                 <p className="text-gray-600 mt-1">貼上您的文案，AI 將為您生成一系列精彩配圖！</p>
            </header>
                         ...
                         {generatedImages.length > 0 && (
                             <div className="flex items-center gap-4">
                                <div className="flex items-center">
                                    ...
                                    />
                                    <label htmlFor="selectAll" className="ml-2 text-sm text-gray-700">全選</label>
                                </div>
                                <button 
